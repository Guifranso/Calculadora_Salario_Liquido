...
from flask import Flask, render_template, jsonify , json, request, redirect, session, flash, url_for
import sqlite3
import json
...
def calcular():
    ...
    salarioBruto = float(request_data['salarioBruto'])
    desconto = float(request_data['desconto'])
    dependente = float(request_data['dependente'])
